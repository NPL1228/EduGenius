    ...
    return (
        <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[80%]`}>
                ...
                <ClientOnlyTime timestamp={message.timestamp} isUser={isUser} />
            </div>
        </div>
    );
}

function ClientOnlyTime({ timestamp, isUser }: { timestamp: Date, isUser: boolean }) {
    ...
    if (!mounted) return <span className={`text-xs mt-2 block h-4 ${isUser ? 'text-white/70' : 'text-gray-400'}`}></span>;

    // Don't show time for welcome message (timestamp is epoch)
